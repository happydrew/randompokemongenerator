...
// 处理单个 Pokemon
async function processPokemon(page, pokemon) {
  try {
    ...
    // 等待页面加载（Cloudflare 验证）
    await page.waitForTimeout(3000);

    // 提取图片 URL
    const imageUrl = await page.evaluate((variant) => {
    ...
    console.log(`  Found image: ${fullImageUrl}`);
...
async function main() {
  ...
  for (const pokemon of missingPokemon) {
    const result = await processPokemon(page, pokemon);
    results.push(result);
