...
import { DisplayPokemon, displayPokemon } from "./pokemon";
...
let shiniesCahce: DisplayPokemon[];
let shinyMapCache: Map<string, DisplayPokemon>;
// 保存的shiny数量
...
function toggleHistoryVisibility() {
	...
	document.getElementById("previous")?.classList.toggle("invisible", displayedIndex >= latestPokemon.length - 1);
	document.getElementById("next")?.classList.toggle("invisible", displayedIndex <= 0);
...
function toggleShinyDisplay(forceInvisible?: boolean) {
	const isInvisible = document.getElementById("shiny-container")?.classList.toggle("hidden", forceInvisible);
...
function loadShinies() {
	...
	if (shiniesCache) {
		shiniesCahce = JSON.parse(shiniesCache).map((s: DisplayPokemon) => new DisplayPokemon(s));
