...
function addResetButtonListener() {
    ...
    document.getElementById("filter-options-form")!.addEventListener('reset', event => {
        ...
        modal.innerHTML = `
            <p>Are you sure to <strong>Reset all filter options</strong>?<br>This will set all filter options to their default state.</p>
        ...
        document.querySelector(".generator-tool")!.appendChild(overlay);

        function removeModal() {
            document.querySelector(".generator-tool")!.removeChild(overlay);
        }
    });
    ...
    document.getElementById("show-options-form")?.addEventListener('reset', event => {
        ...
        modal.className = 'modal';
        modal.innerHTML = `
            <p>Are you sure to <strong>Reset all show options</strong>?<br>This will set all show options to their default state.</p>
            <button id="confirmBtn">Yes</button>
            <button id="cancelBtn">No</button>
        `;
...
async function wakeUpBackend() {
    try {
        ...
        console.error("backend queryConditions wakeup failed", error)
    }
    ...
    try {
        ...
        console.error("backend pokemon-details wakeup failed", error)
...
function jsonObjToUrlParams(obj: any): string {
    return Object.entries<any>(obj).map(([key, value]) => key.toString() + "=" + value.toString())
        .join("&");
}
...
function getOptionsFromForm(): Options {
    ...
    // filter params
    {
        ...
        // eggGroups
        const eggGroups = getDropdownOptions("eggGroup");
        if (eggGroups) {
    ...
    showParams.n = (document.getElementById("n") as HTMLSelectElement).value;
...
function loadOptions() {
    const options: Options = { filterParams: {}, showParams: {} }
    // first load options from cache
    const cachedOptions = loadOptionsFromCache();
    if (cachedOptions) {
        if (cachedOptions.filterParams) {
            Object.assign(options.filterParams, cachedOptions.filterParams);
        }
        if (cachedOptions.showParams) {
            Object.assign(options.showParams, cachedOptions.showParams);
        }
    }
    // then load options from URL, and override options from cache
    let urlOptions: Options | null = null;
    if (urlHasOptions()) {
        try {
            urlOptions = convertUrlParamsToOptions();
        } catch (error) {
            console.error("解析url参数失败：", error);
        }
    }
    if (urlOptions) {
        if (urlOptions.filterParams) {
            Object.assign(options.filterParams, urlOptions.filterParams);
        }
        if (urlOptions.showParams) {
            Object.assign(options.showParams, urlOptions.showParams);
        }
    }
    setOptions(options);
}

function loadOptionsFromCache(): Options | null {
...
function setOptions(options: Options) {
    ...
    if (options.filterParams) {
        ...
        if (options.filterParams.eggGroups != undefined) {
            setDropdownIfValid("eggGroup", options.filterParams.eggGroups);
        }
...
function convertUrlParamsToOptions(): Options {
...
function addFormChangeListeners() {
    ...
    document.querySelectorAll(".dropdown").forEach((dropdown: Element) => {
        updateDropdownTitle(dropdown as HTMLElement);
        dropdown.querySelectorAll("input[type='checkbox']").forEach(checkbox => {
            checkbox.addEventListener("change", () => updateDropdownTitle(dropdown as HTMLElement));
        });
    });
...
let currentClickTooltip: HTMLElement | null = null;
let currentClickTipElement: HTMLElement | null = null;
let currentHoverToolTip: HTMLElement | null = null;
let currentHoverTipElement: HTMLElement | null = null;
...
function addClickTipListeners() {
    ...
    document.addEventListener('click', (event) => {
        if (currentClickTooltip && !currentClickTooltip.contains(event.target as HTMLElement)
            && currentClickTipElement && !currentClickTipElement.contains(event.target as HTMLElement)) {
 ...
 */
//@ts-expect-error
function processClickTipEvent(clickElement: HTMLElement, event) {
    ...
    if (currentClickTipElement && currentClickTipElement == event.currentTarget && currentClickTooltip) {
        currentClickTooltip.remove();
        currentClickTooltip = null;
        currentClickTipElement = null;
        return;
    ...
    document.querySelector(".generator-tool")!.appendChild(tooltip);
...
function processMouseEnterTipEvent(hoverElement: HTMLElement, event: Event) {
    ...
    // 首先决定提示框的横向位置，即左边缘的位置
    const tooltipLeft = Math.min(hoverEleRect.right - 0.5 * hoverElement.offsetWidth, window.innerWidth - tooltip.offsetWidth) + window.scrollX;
    tooltip.style.left = tooltipLeft + "px";
    // 决定提示框的纵向位置，即上边缘的位置
    // 主元素上下两边，哪边空间更大，就往哪边放
    const hoverUpDistance = hoverEleRect.top;
    const hoverDownDistance = window.innerHeight - hoverEleRect.bottom;
    if (hoverDownDistance > hoverUpDistance) {
        // 下边空间大，往下边放
        const tooltipTop = Math.min(hoverEleRect.bottom, window.innerHeight - tooltip.offsetHeight) + window.scrollY;
        tooltip.style.top = tooltipTop + "px";
    } else {
        // 上边空间大，往上边放
        const tooltipTop = Math.max(hoverEleRect.top, tooltip.offsetHeight) + window.scrollY - tooltip.offsetHeight;
        tooltip.style.top = tooltipTop + "px";
    }
    // 显示提示框
    tooltip.style.visibility = "visible";
